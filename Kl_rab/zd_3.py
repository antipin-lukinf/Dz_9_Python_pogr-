...
def function_json(*args, **kwargs) -> str:
    ...
    if kwargs:
        for key, value in kwargs.items():
            list_kvargs.append(f'{key}={value}')

    result_str = ' '.join(list(map(str, list_for_args))) + ' '.join(list(map(str, list_kvargs)))
...
if __name__ == '__main__':
    print(function_json(1,2,3,4,5))
    print(function_json(text=1,te=2))